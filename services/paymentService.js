...
const GUINEPAY_BASE_URL = process.env.GUINEPAY_BASE_URL || 'https://api.guinepay.com/v1'
...
async function guinePayRequest(method, endpoint, body = null) {
  ...
  if (!res.ok) {
    const error = await res.json().catch(() => ({ message: 'GuinePay API error' }))
    throw new AppError(`GuinePay: ${error.message || 'Payment gateway error'}`, 502)
  }
...
export async function createPaymentIntent(tenantId, data) {
  const { amount, currency = 'GNF', studentId, description, paymentType = 'SCHOOL_FEE', subscriptionId } = data
...
export async function processWebhook(payload, signature, timestamp) {
  ...
  await prisma.payment.update({
    ...
    data: {
      ...
      ...(paidAt && { paidAt }),
      ...(newStatus === 'FAILED' && { failureReason: eventData.failure_reason }),
    },
  })

  // If subscription payment succeeded, activate subscription
  // ── Activate subscription plan on success ─────────────────────────────────
  if (newStatus === 'SUCCESS' && payment.subscriptionId) {
    try {
      ...
      if (sub) {
        ...
        // Notify school admin via WhatsApp
        try {
    ...
    } catch (e) { console.error('[Webhook] Plan activation error:', e.message) }
  }
  ...
  if (newStatus === 'SUCCESS' && payment.studentId) {
    ...
    } catch (e) { console.error('[WA Payment lookup]', e.message) }
  }
...
export const SUBSCRIPTION_PLANS = {
  ...
  BASIC: {
    ...
    maxStudents: 200,
    maxTeachers: 20,
    features:    ['Up to 200 students', '20 teachers', 'Full reports', 'Payment tracking'],
  },
  PRO: {
    name:        'Pro',
    price:       1500000,
    maxStudents: 1000,
    maxTeachers: 100,
    features:    ['Up to 1000 students', '100 teachers', 'Advanced analytics', 'Priority support', 'Custom branding'],
  },
  ENTERPRISE: {
    name:        'Enterprise',
    price:       5000000,
    maxStudents: 999999,
    maxTeachers: 999999,
    features:    ['Unlimited students', 'Unlimited teachers', 'Dedicated support', 'SLA', 'Custom integrations'],
  },
}
