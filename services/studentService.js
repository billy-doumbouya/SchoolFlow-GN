...
export async function createStudent(tenantId, data) {
  ...
  const limit = subscription?.maxStudents || plan.maxStudents
  if (studentCount >= limit) {
    throw new AppError(
      `Quota atteint — votre plan ${plan.label} est limité à ${limit} élèves actifs. Upgradez votre abonnement dans Facturation.`,
      403
    )
  }
  // ──────────────────────────────────────────────────────────────────────────
