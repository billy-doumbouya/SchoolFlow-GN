...
export async function GET(request) {
  try {
    ...
    if (!hasPermission(role, PERMISSIONS.PAYMENT_READ)) return errorResponse('Forbidden', 403)

    const { payments, pagination } = await listPayments(tenantId, request.nextUrl.searchParams)
...
export async function POST(request) {
  try {
    ...
    const allowedRoles = [PERMISSIONS.PAYMENT_CREATE, PERMISSIONS.PAYMENT_MANAGE]
    if (!allowedRoles.some(p => hasPermission(role, p))) {
      return errorResponse('Forbidden', 403)
    }

    const body = await request.json()
    const data = await createPaymentSchema.validate(body, { abortEarly: false })
